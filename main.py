...
class CalendarPlugin(Star):
    ...
    def parse_ics_to_dict(self, text):
        ...
        for block in vevent_blocks:
            ...
            dtstamp = re.search(r"^DTSTAMP:(.*?)$", block, re.M) # 关键：发布/更新时间
            ...
            t_stamp = dtstamp.group(1).strip() if dtstamp else ""
    ...
    def draw_card(self, draw, base_img, x, y, ev, fonts):
        ...
        # 颜色控制
        m_clr = "#BDC3C7" if is_canceled else self.get_color(ev["url"])
        text_alpha = 150 if is_canceled else 255
        
        draw.line([x - 15, y + 10, x - 15, y + 180], fill="#DCDFE6", width=2)
        
        time_str = datetime.strptime(ev["time"], "%Y-%m-%d %H:%M:%S").strftime('%H:%M')
        draw.text((x, y), time_str, fill="#99A2AA", font=fonts['time'])
        
        title = ev["title"]
        if is_canceled: title = "[失效/调整] " + title
        ...
        tag_draw.rounded_rectangle([x+10, y_c+15, x+65, y_c+43], radius=8, fill=(255, 255, 255, 60 if not is_canceled else 30))
        base_img.paste(tag_canvas, (0, 0), tag_canvas)
        
        # 绘制文字
        text_fill = (255, 255, 255, text_alpha)
        draw.text((x + 18, y_c + 18), ev["tag"], fill=text_fill, font=fonts['tag'])
        draw.text((x + 75, y_c + 16), ev["name"], fill=text_fill, font=fonts['name'])
        for i, line in enumerate(lines):
            draw.text((x + 10, y_c + 50 + i * 25), line, fill=text_fill, font=fonts['title'])
            
        # 如果是调整后的，画删除线
        if is_canceled:
            line_y_mid = y_c + card_h // 2 + 5
            draw.line([x + 5, line_y_mid, x + COL_W - 35, line_y_mid], fill="#7F8C8D", width=3)
            # 时间也划掉
            draw.line([x, y + 12, x + 55, y + 12], fill="#99A2AA", width=2)
    ...
    async def update_calendar_image(self, week_offset=0):
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                # 双重循环对比（针对相近时间的同一直播间进行去重）
                for i in range(len(render_list)):
                    for j in range(len(render_list)):
                        if i == j: continue
                        ev1, ev2 = render_list[i], render_list[j]
                        
                        # 判定为“同一场直播”的条件：
                        # 1. 直播间URL相同
                        # 2. 角色名相同
                        # 3. 两个直播时间点相差在 48 小时以内
                        if ev1["url"] == ev2["url"] and ev1["name"] == ev2["name"]:
                            t1 = datetime.strptime(ev1["time"], "%Y-%m-%d %H:%M:%S")
                            t2 = datetime.strptime(ev2["time"], "%Y-%m-%d %H:%M:%S")
                            if abs((t1 - t2).total_seconds()) < 172800: # 48小时
                                # 比较 STAMP，旧的标记为 canceled
                                if ev1["stamp"] < ev2["stamp"]:
                                    ev1["canceled"] = True
                                elif ev1["stamp"] == ev2["stamp"] and t1 != t2:
                                    # 如果STAMP一样但时间不同，UID里日期较早的通常是旧的
                                    if ev1["uid"] < ev2["uid"]: ev1["canceled"] = True
            ...
            img_h = MT + max(day_heights + [100]) + 60
            ...
            for i in range(7):
                ...
                y_o = MT
                # 排序：让被取消的排在下面，正常的排在上面（可选）
                day_events = sorted(w_data[i], key=lambda x: (x["canceled"], x["time"]))
